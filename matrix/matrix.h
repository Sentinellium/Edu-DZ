...
class matrix {
    ...
    T **data;
    public:
    //we use differnt constructors for different types of input data

    //constructor without inpit data
    matrix() : data(new T*[M]) {
        for (size_t i(0); i < M; ++i)
            data[i] = new T[N];
    }
    //constructor filling with value
    matrix(const T &value) : matrix() {
        for (size_t i(0); i < M; ++i)
            for (size_t j(0); j < N; ++j)
                data[i][j] = value;
    }
    //constructor copying the t matrix
